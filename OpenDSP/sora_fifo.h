...
class SoraCounterFifo
{
    ...
    FILE *dump;
public:
    SoraCounterFifo(const char *dump_file = NULL)
...
public:
    ...
    FILE *dump;
public:
    SoraLinearFifo(const char *dump_file = NULL)
        : w_count(0), r_count(0), w_itCounter(0), r_itCounter(0)
        , w_pointer(data), r_pointer(data)
    {
#if defined(USER_MODE) && defined(BBB_VALIDATION)
    ...
    void RFlush()
    {
        ...
        {
            memcpy(r_pointer - FIELD_SIZE / 2, r_pointer, w_pointer - r_pointer);
