...
void CPlotWnd::PlotLine ( double * pData, int nSize ) {
    ...
    HPEN hTickPen      = CreatePen ( PS_DASH, 2, RGB(0xcc,0xcc,0xcc) );
    ...
    char labelTxt [120];
    double yunit;
    int yunitcnt = h / m_nFontHeight / 2;

    SelectObject (m_hBkDC, hTickPen );
	// Draw x-axis
    for (int i = 0; i < yunitcnt; i++)
    {
        yunit = data_span / yunitcnt;
        y     = m_nMargin + (int) ((1 + data_min / data_span)*h*i/yunitcnt );
        sprintf ( labelTxt, "%.3e", yunit * (yunitcnt - i) );
        TextOut ( m_hBkDC, 0, y, labelTxt, strlen (labelTxt) );
